...
#ifndef APPLICATIONS_DYMO_DYMO_DATAQUEUE_H
#define APPLICATIONS_DYMO_DYMO_DATAQUEUE_H
...
class DYMO_QueuedData {
	public:
		DYMO_QueuedData(const IPDatagram* datagram, IPAddress destAddr) : datagram(datagram), destAddr(destAddr) {}

		const IPDatagram* datagram;
		IPAddress destAddr;
...
class DYMO_DataQueue : public cObject {
	public:
		DYMO_DataQueue(IP* ipLayer, int BUFFER_SIZE_PACKETS, int BUFFER_SIZE_BYTES);
		~DYMO_DataQueue();
	...
	protected:
		...
		IP* ipLayer; /**< pointer to IP layer module */
		...
		void reinjectDatagramsTo(IPAddress destAddr, int prefix, IP::Hook::Result verdict);
