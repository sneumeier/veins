...
#include "applications/dymo/DYMO.h"
#include "UDPControlInfo_m.h"
#include "UDPSocket.h"
#include "IPProtocolId_m.h"
#include "IPControlInfo.h"
#include "UDPPacket.h"
#include "base/ModuleAccess.h"
#include "IP.h"
...
namespace {
	...
	const int UDPPort = 9000; /**< UDP Port to listen on (TBD) */
...
void DYMO::initialize(int aStage) {
	...
	if (2 == aStage) {
		...
		IP* ipLayer = check_and_cast<IP*>(findModuleWherever("ip", simulation.getContextModule()));
		ipLayer->registerHook(0, this);
		queuedDataPackets = new DYMO_DataQueue(ipLayer, BUFFER_SIZE_PACKETS, BUFFER_SIZE_BYTES);
		WATCH_PTR(queuedDataPackets);
	}
}

void DYMO::setupLowerLayer() {
	ev << "Binding to UDP port " << UDPPort << endl;

	cMessage *msg = new cMessage("UDP_C_BIND", UDP_C_BIND);
	UDPControlInfo *ctrl = new UDPControlInfo();
	ctrl->setSrcPort(UDPPort);
...
IP::Hook::Result DYMO::datagramLocalOutHook(const IPDatagram* datagram, const InterfaceEntry* outIE, const IP* ipLayer) {
	Enter_Method("onOutboundDataPacket(%s)", datagram->getName());

	IPAddress destAddr = datagram->getDestAddress();
	int TargetSeqNum=0;
	int TargetHopCount=0;

	// skip route discovery if this packet is for us, or if packet is to be broadcasted/multicasted
	if ((destAddr.getInt() == myAddr) || (destAddr == IPAddress::ALLONES_ADDRESS) || (destAddr.isMulticast())) {
	...
	return IP::Hook::QUEUE;
}

IP::Hook::Result DYMO::datagramPreRoutingHook(const IPDatagram* datagram, const InterfaceEntry* inIE, const IP* ipLayer) {
	Enter_Method("datagramPreRoutingHook(%s)", datagram->getName());
	...
	if (isDymoPacket) return IP::Hook::ACCEPT;
	...
	if (destAddr.getInt() == myAddr) {
		...
		dataLatency += simTime() - datagram->getCreationTime();
		dataSamples++;
		return IP::Hook::ACCEPT;
	}

	// if this is a broadcast or multicast, everything is fine
	if ((destAddr == IPAddress::ALLONES_ADDRESS) || (destAddr.isMulticast())) return IP::Hook::ACCEPT;
...
IP::Hook::Result DYMO::datagramPostRoutingHook(const IPDatagram* datagram, const InterfaceEntry* inIE, const InterfaceEntry* outIE, const IPAddress& nextHopAddr, const IP* ipLayer) {
	return IP::Hook::ACCEPT;
}
...
InterfaceEntry* DYMO::getNextHopInterface(DYMO_PacketBBMessage* pkt) {
	...
	if (!controlInfo) error("received packet did not have UDPControlInfo attached");
	...
	if (!srcHost) error("parent module not found");

	IInterfaceTable* srcIftable = check_and_cast<IInterfaceTable*>(srcHost->getSubmodule("interfaceTable"));
	if (!srcIftable) error("parent module has no interface table \"interfaceTable\"");

	InterfaceEntry* srcIf = srcIftable->getInterfaceById(interfaceId);
...
void DYMO::handleLowerRERR(DYMO_RERR *my_rerr) {
	...
	IPAddress sourceAddr = controlInfo->getSrcAddr().get4();
...
void DYMO::sendDown(cPacket* apMsg, int destAddr) {
	...
	//TODO: setTimeToLive ?
	ctrl->setSrcPort(UDPPort);
	ctrl->setDestAddr(IPAddress(destAddr));
	ctrl->setDestPort(UDPPort);
	delete(apMsg->removeControlInfo());
	apMsg->setControlInfo(ctrl);
	...
	sendDelayed(apMsg, jitter, "udp$o");
}
