...
#include "applications/dymo/DYMOAccess.h"
#include "IPv4InterfaceData.h"
#include "Ieee802Ctrl_m.h"
#include "IPControlInfo.h"
#include "networklayer/ipv4/IP.h"
...
DYMO_DataQueue::DYMO_DataQueue(IP* ipLayer, int BUFFER_SIZE_PACKETS, int BUFFER_SIZE_BYTES) : dataQueueByteSize(0), ipLayer(ipLayer), BUFFER_SIZE_PACKETS(BUFFER_SIZE_PACKETS), BUFFER_SIZE_BYTES(BUFFER_SIZE_BYTES) {
}
...
void DYMO_DataQueue::reinjectDatagramsTo(IPAddress destAddr, int prefix, IP::Hook::Result verdict) {
	bool tryAgain = true;
	while (tryAgain) {
		tryAgain = false;
		for (std::list<DYMO_QueuedData>::iterator iter = dataQueue.begin(); iter != dataQueue.end(); iter++) {
			...
			if (qd.destAddr.prefixMatches(destAddr, prefix)) {
				...
				ipLayer->reinjectDatagram(qd.datagram, verdict);
				tryAgain = true;
				break;
			}
		}
...
void DYMO_DataQueue::dequeuePacketsTo(IPAddress destAddr, int prefix) {
	reinjectDatagramsTo(destAddr, prefix, IP::Hook::ACCEPT);
}

void DYMO_DataQueue::dropPacketsTo(IPAddress destAddr, int prefix) {
	reinjectDatagramsTo(destAddr, prefix, IP::Hook::DROP);
}

IP* DYMO_DataQueue::getIpLayer() {
	return ipLayer;
}
